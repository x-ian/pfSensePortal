...
if [ $CHECK_REPLY -eq 1 ] && [ $CHECK_ACCESS -eq 0 ]; then
  ...
  echo "$MAC - $IP - 1 - device not yet registered - `date +%Y%m%d-%H%M%S`" >> /home/check_device_status.log
  exit 1
fi

echo $RADTEST | grep "Too many users"
if [ $? -eq 0 ]; then
  echo "Too many users. Please try again later."
  echo "$MAC - $IP - 2 - Too many users - `date +%Y%m%d-%H%M%S`" >> /home/check_device_status.log
  exit 2
fi

echo $RADTEST | grep "usage time has been reached"
if [ $? -eq 0 ]; then
  echo "Data volume reached"
  echo "$MAC - $IP - 3 - Data volume reached - `date +%Y%m%d-%H%M%S`" >> /home/check_device_status.log
  exit 3
fi
...
if [ $? -eq 0 ]; then
  ...
  echo "$MAC - $IP - 4 - Device permanently disabled - `date +%Y%m%d-%H%M%S`" >> /home/check_device_status.log
  exit 4
fi

echo $RADTEST | grep "Data bundle during business hours exceeded"
if [ $? -eq 0 ]; then
  echo "Data bundle during business hours exceeded."
  echo "$MAC - $IP - 5 - Data bundle during business hours exceeded - `date +%Y%m%d-%H%M%S`" >> /home/check_device_status.log
  exit 5
fi
...
if [ $? -eq 0 ]; then
  ...
  echo "$MAC - $IP - 0 - enabled and active - `date +%Y%m%d-%H%M%S`" >> /home/check_device_status.log
  exit 0
fi
...
echo "$MAC - $IP - 9 - RADIUS offline or unknown response - `date +%Y%m%d-%H%M%S`" >> /home/check_device_status.log
exit 9
