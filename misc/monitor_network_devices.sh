...
if [ -e $LOG ]; then
  echo "" >> $LOG
  echo "(mail generated by script pfSense:///home/pfSensePortal/misc/monitor_network_devices.sh)" >> $LOG
  echo "(list of monitored devices under pfSense://home/pfSensePortal/misc/monitor_network_devices.txt)" >> $LOG
  SUBJECT="`echo "172.16.1.2 - pfSense: Internal network devices check : "` `date +%Y%m%d-%H%M`"
